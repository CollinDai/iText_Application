 ...
 *
 */
public class TestClassAttendList7 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		OutputStream out = new FileOutputStream("出席簿7.pdf");
		...
		for (float i=0, j=39.4f; i<4; ++i) {
			monthchar.setPosition(j+=10.4f, 18);
		...
		pdfdoc.headerOrPagenumber(_text, new float[][] { { 10, 7 },
				{ 79.5f, 6f }, { 7, 12 }, { 12.5f, 12 }, { 19.5f, 12 },
				{ 59.5f, 12 }, { 68.5f, 12 }, { 85, 12 }, { 7, 16 },
				{ 16, 16 }, { 59.5f, 16 }, { 68.5f, 16 },
				{ 50.1f, 20.5f }, { 60.7f, 20.5f }, { 70.6f, 20.5f }, { 80.9f, 20.5f } },
				72f);
		
		// add table
		PdfTable pdftable = new PdfTable(22, new float[] { 0.8f, 1.7f, 3.5f,
				4.3f, 2.4f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f,
				0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 0.8f, 3 });
		String[] heads = { "No.", "会員番号", "氏　　名", "ふ り が な", "生年月日", "", "", "",
				"", "", "", "", "", "", "", "", "", "", "", "", "", "有効年月" };
		...
		// add table contents
		pdftable.setColumnAlignment(new int[] { 5, 5, 5, 5, 5, 5, 5, 1,
				1, 5, 5, 1, 1, 5, 5, 1, 1, 5, 5, 1, 1, 4 });

		int objnumber = 25;

		String[] testobject = { "1284", "劉德華", "アンディラウ", "1965年 2月14日", "平成23年15月" };
		for (int i = 0; i < objnumber; ++i) {
			for (int j = 0; j < 22; ++j) {
				if (j == 0)
					pdftable.addCell(Integer.toString(i + 1), 21.25f, 5);
				else if (j == 21)
					pdftable.addCell(testobject[4], 21.25f, 4);
				else if (j > 4 && j < 21)
